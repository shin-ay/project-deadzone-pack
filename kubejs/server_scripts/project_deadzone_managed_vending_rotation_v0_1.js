...
const DZ_VM_MARKETS = {
  base:{name:'PDZ Base Supply Market',fixed:[
    ...
    sale:[{d:'sale',i:'minecraft:bread',n:6,p:3,l:12},{d:'sale',i:'minecraft:cooked_beef',n:4,p:5,l:8},{d:'sale',i:'minecraft:torch',n:32,p:4,l:8},{d:'sale',i:'tacz:ammo',n:20,p:8,l:6},{d:'sale',i:'minecraft:iron_ingot',n:4,p:6,l:6}],
...
function dzVmNativeStack(id,count){let item=DZ_VM_FORGE_REGISTRIES.ITEMS.getValue(new DZ_VM_RESOURCE_LOCATION(String(id)));return item?new DZ_VM_ITEM_STACK(item,Math.max(1,Math.floor(count))):DZ_VM_ITEM_STACK.EMPTY}
...
function dzVmConfigure(server,entry,force){
  ...
  let block=level.getBlock(entry.x,entry.y,entry.z);if(String(block.id)!=='lightmanscurrency:vending_machine'&&String(block.id)!=='lightmanscurrency:vending_machine_large')return false
  if(!force&&Number(entry.next||0)>Date.now())return true
  try{
    let be=level.getBlockEntity(pos),trader=be?be.getTraderData():null;if(!trader)return false
    ...
    for(let i=0;i<12;i++){
      ...
      trade.setItem(dzVmNativeStack(o.i,o.n),0);trade.setEnforceNBT(0,false);trade.setCustomName(0,o.i);trade.setCost(dzCreditValue(o.p))
    ...
    entry.next=Date.now()+DZ_VM_INTERVAL;entry.version=1
    console.info('[PDZ Market] rotated '+entry.market+' at '+entry.dim+' '+entry.x+','+entry.y+','+entry.z)
    return true
  }catch(e){console.error('[PDZ Market] rotation failed: '+e);return false}
}
