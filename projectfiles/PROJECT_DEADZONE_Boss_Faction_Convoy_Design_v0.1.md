...
- 登録済み拠点の96m以内へ入ると、現在の所有勢力に対応する守備隊を実体化する。
- 144mより離れると守備隊を仮想化し、チャンク常駐とサーバー負荷を避ける。
- 小規模3人・中規模5人・大規模7人を基準とし、将校・衛生兵・重装兵を勢力別編成へ含める。
- 守備隊を全滅させた場合は15分の再配置待機時間を設ける。出入りだけで即時復活しない。
- 拠点が占領された場合は、次の実体化から新しい所有勢力の守備隊へ切り替える。
- 交易属性を持つ友好・中立拠点では、重複を防止しながら商人も配置する。
- 管理コマンド: `/deadzonegarrison pulse`、`/deadzonegarrison reset_near`
